...
const ColorPicker = ({ selectedColor, onChange }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="grid grid-cols-4 absolute top-full mt-2 p-3 gap-2 w-56 bg-white border border-gray-200 rounded-md shadow-sm z-10">
          {colors.map((color) => (
            <div
              key={color.value}
            ...
            >
              ...
              <p className="text-xs text-center mt-1 text-gray-600 whitespace-nowrap">
                {color.name}
              </p>
            </div>
          ))}
